...
function FacedetectionTF({backendName, modelPath}: ModelInfo) {
    ...
    const drawResult = useCallback<(image: tf.Tensor, score: tf.Tensor, bbox: tf.Tensor, canvasHeight: number, canvasWidth: number,) => void>(async (image, score, bbox, canvasHeight, canvasWidth) => {
        ...
        const resizeImage = imageAlpha.resizeBilinear([canvasHeight, canvasWidth,]);
        ...
        finalBox.forEach((value: number, index: number) => {
            ...
            if (index % 2 === 0) finalBox[index] *= wRatio; else finalBox[index] *= hRatio;
        });
    ...
    const inference = useCallback<(path: string, select: boolean) => void>(async (path, select) => {
        ...
        if (inferenceRef.current && cameraCount > 0) {
            ...
            if (deviceId !== '') webcam = await tf.data.webcam(videoRef.current, {
                deviceId, resizeWidth: 448, resizeHeight: 448, facingMode: !select ? 'user' : 'environment',
            }); else webcam = await tf.data.webcam(videoRef.current, {
                resizeWidth: 448, resizeHeight: 448, facingMode: !select ? 'user' : 'environment',
            });
            ...
            while (inferenceRef.current) {
                ...
                if (inferenceRef.current) {
                    ...
                    await drawResult(img.clone(), scores.clone(), bboxes.clone(), canvasHeight, canvasWidth,);
