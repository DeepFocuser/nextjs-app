...
function HumanmattingBetterONNX({modelPath}: { modelPath: string }) {
    ...
    const inference = useCallback<(modelPath: string, select: boolean) => void>(async (modelPath, select) => {
        ...
        if (inferenceRef.current && cameraCount > 0) {
            ...
            if (deviceId !== '') {
                constraints = {
                    video: {
                        deviceId, facingMode: !select ? 'user' : 'environment',
                    },
                };
            } else {
                constraints = {
            ...
            try {
                ...
                if (stream) {
                    const infernceContext = canvasInferenceRef.current?.getContext('2d', {
                    ...
                    feeds[session.inputNames[1]] = new Tensor(new Float32Array(1 * 64 * 24 * 24), [1, 64, 24, 24],);
                    feeds[session.inputNames[2]] = new Tensor(new Float32Array(1 * 40 * 48 * 48), [1, 40, 48, 48],);
                    feeds[session.inputNames[3]] = new Tensor(new Float32Array(1 * 20 * 96 * 96), [1, 20, 96, 96],);
                    feeds[session.inputNames[4]] = new Tensor(new Float32Array(1 * 16 * 192 * 192), [1, 16, 192, 192],);

                    const drawCanvas = async () => {
                        if (inferenceRef.current) {
                            const targetCanvasHeight = canvasResultRef.current?.height;
                            const targetCanvasWidth = canvasResultRef.current?.width;

                            infernceContext.clearRect(0, 0, targetWidth, targetHeight,);
                            infernceContext.drawImage(videoRef.current, 0, 0, targetWidth, targetHeight,);
                            const captureImage = infernceContext.getImageData(0, 0, targetWidth, targetHeight,);
                        ...
                        } else if (canvasResultRef.current !== null) canvasResultRef.current.style.display = 'none';
                    };
                    await drawCanvas();
                }
            } catch (error) {
    ...
    useEffect(() => {
        const windowResizeListener = () => {
            ...
            canvasResultRef.current.height = Math.floor(window.innerHeight * 0.5,);
        };
